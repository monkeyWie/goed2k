...
func (m *Mapping) setAddress(id string, address Address) {
	...
	if existing, ok := m.extAddresses[id]; !ok || !existing.Equal(address) {
		logx.Info(fmt.Sprintf("New NAT port mapping: external %s address %s to local address %s.", m.protocol, address, m.address))
...
func (m *Mapping) removeAddress(id string) {
	...
	if ok {
		logx.Info(fmt.Sprintf("Removing NAT port mapping: external %s address %s, NAT %s is no longer available.", m.protocol, addr, id))
...
func (m *Mapping) clearAddresses() {
	...
	for id, addr := range m.extAddresses {
		logx.Info(fmt.Sprintf("Clearing mapping %s: ID: %s Address: %s", m, id, addr))
